			  ...
			  <tbody>
			    ...
			    <tr onclick="showdet('<?=$clm['EmployeeID']?>')" style="<?=$trcolor?>">
					...
					<td style="text-align: left;">
						<input type="checkbox" onclick="attachthis(this,'<?=$_REQUEST['expid']?>','<?=$clm['ExpId']?>')" <?php if($_REQUEST['expid']==$clm['AttachTo'] OR $clm['AttachTo']>0){echo 'checked';} ?>>

						<span id="res<?=$clm['ExpId']?>">
					  	
					  	<?php if($_REQUEST['expid']==$clm['AttachTo']){ ?>
					  	 <div class='btn btn-sm btn-success'>Attached from this </div>
						<?php } elseif($clm['AttachTo']>0){ ?>
					  	 <div class='btn btn-sm btn-success'>Attached from Other </div>
						<?php } ?>
