...
$rp= mysql_query("SELECT EmployeeID from hrm_employee_reporting where AppraiserId=".$_SESSION['EmployeeID'],$con2); $rows=mysql_num_rows($rp); 
     if($rows>0)
     {  
       while($rpv=mysql_fetch_array($rp)){ $array_e[]=$rpv['EmployeeID']; } 
       $ee = implode(',', $array_e);    
     } 
...
<div id="apprdiv" class="col-lg-9 col-md-12 h-100" style="border-left:5px solid #d9d9d9;<?php if($_SESSION['EmpRole']=='A'){echo 'display:none;';} ?> ">
 ...
 &nbsp;
  <?php $svs=mysql_query("select MoveRep from hrm_employee where EmployeeID=".$_SESSION['EmployeeID'],$con2); 
      $resv=mysql_fetch_assoc($svs); 
      if($resv['MoveRep']=='Y'){ ?>
	 <a class="btn btn-sm btn-warning" href="https://www.vnress.in/expense/RepIndxHome.php?ID=<?=$_SESSION['EmployeeID']?>&YI=<?=$_SESSION['FYearId']?>" target="_blank">Other Team Group</a> 
  <?php } ?>
...
<table class="estable table shadow ">
 ...
 <tbody>
 <?php $e=mysql_query("SELECT f.* FROM `y".$_SESSION['FYearId']."_monthexpensefinal` f WHERE f.EmployeeID in ($ee) and YearId=".$_SESSION['FYearId']." and `Status`='Closed' and Total_Claim>0 and Verified_Amount>0 and Verified_Date!='0000-00-00' and (Approved_Amount=0 OR Approved_Date='0000-00-00') order by Month asc, EmployeeID");  
  $i=1;
  
  //echo "SELECT f.* FROM `y".$_SESSION['FYearId']."_monthexpensefinal` f WHERE f.EmployeeID in ($ee) and YearId=".$_SESSION['FYearId']." and `Status`='Closed' and Total_Claim>0 and Verified_Amount>0 and Verified_Date!='0000-00-00' and (Approved_Amount=0 OR Approved_Date='0000-00-00') order by Month asc, EmployeeID";
...
?>
  ...
  <td style="text-align:center;">
   <?php if($enumd['Verified_Amount']>0 && $enumd['Verified_Date']!='' && $enumd['Verified_Date']!='0000-00-00' && $enumd['Verified_Date']!='1970-01-01'){ echo floatval($enumd['Verified_Amount']); } 
   ?>
   </td>
...
$from = 0;
  ...
  $ea=mysql_query("SELECT f.* FROM `y".$_SESSION['FYearId']."_monthexpensefinal` f WHERE f.EmployeeID in ($ee) and YearId=".$_SESSION['FYearId']." and `Status`='Closed' and Total_Claim>0 and Verified_Amount>0 and Verified_Date!='0000-00-00' and Approved_Amount>0 and Approved_Date!='0000-00-00' order by Month asc, EmployeeID");  // LIMIT ".$from.",".$offset
 ...
 <tr>
  ...
  <td style="text-align:center;">
   <?php if($eadet['Verified_Amount']>0 && $eadet['Verified_Date']!='' && $eadet['Verified_Date']!='0000-00-00' && $eadet['Verified_Date']!='1970-01-01'){ echo floatval($eadet['Verified_Amount']); } 
   ?>
   </td>
  <td style="text-align:center;">
...
<script type="text/javascript">
	...
	function showmonthdet(month,sts,emp,csts){ 
		...
		if(status.value=='close'){ 
			$.post("claimlistajax.php",{act:"monthdettoapprover",month:month,sts:sts,csts:csts,emp:emp},function(data){
	...
	function submittofinance(month,crby){
		...
		if (confirm('Are you sure to Final Submit this month claims to Finance?')){
			$.post("homeajax.php",{"act":"submittofinance","month":month,"crby":crby},function(data){ //alert(data);
				console.log(data);
	...
	function submitFtoFreturn(month,crby){
		if (confirm('Are you sure to return this month claim from approver level?')){
			$.post("homeajax.php",{act:"submitFtoFreturn",month:month,crby:crby},function(data){ alert(data);
	...
	function approveClaim(expid){
		...
		$.post("claimajax.php",{act:"approveClaim",expid:expid,atamt:a,apprtremark:r},function(data){
