...
<?php $stupTFn=mysql_query("select count(*) as TotupTFn from `y".$_SESSION['FYearId']."_expenseclaims` where ApprBy>0 AND ApprDate!='0000-00-00' AND ApprDate!='1970-01-01' AND FinancedBy=0 AND (FinancedDate='0000-00-00' OR FinancedDate='1970-01-01') AND CrBy=".$_REQUEST['ei']." AND ClaimMonth=".$resym['Month']." AND ClaimStatus!='Deactivate'"); $rtupTFn=mysql_fetch_assoc($stupTFn); ?>	
			...
			<?php if($_SESSION['EmpRole']!='OO') { ?>
			
			<td style="text-align:center;vertical-align:middle;"><button type="button" class="btn btn-sm btn-warning" onclick="submittoreturn('<?=$resym['Month']?>','<?=$_REQUEST['ei']?>')" <?php if($rtupTA['TotupTA']>0 OR $rtupTFn['TotupTFn']>0){echo 'disabled';}?>>Return</button>
			<?php } ?>
			
			<?php /*<button type="button" class="btn btn-sm btn-warning" onclick="submittoreturn('<?=$resym['Month']?>','<?=$_REQUEST['ei']?>')" >Return</button>*/?>
