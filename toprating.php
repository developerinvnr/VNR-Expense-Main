	...
	<div class="table-responsive d-flex justify-content-center align-items-center">
	 <table class="table shadow table-responsive" id="ReportTable" style="width:100%;">
	  ...
	  <?php 
	    ...
	    $seleq=mysql_query("SELECT count(*) as Total, CrBy FROM `y".$_SESSION['FYearId']."_expenseclaims` e where `CrDate`=`BillDate` and e.BillDate between '".$f."' and '".$t."' and e.ClaimStatus!='Deactivate' and e.ClaimStatus!='Draft' and e.ClaimId!=0 group by `CrBy` order by total desc"); //and e.ClaimId!=19 and e.ClaimId!=20
		$i=1;
		while($exp=mysql_fetch_assoc($seleq)){
		?>
		
		<?php $u=mysql_query("SELECT EmpCode,Fname,Sname,Lname,DepartmentName FROM `hrm_employee` e inner join hrm_employee_general g on e.EmployeeID=g.EmployeeID inner join hrm_department d on g.DepartmentId=d.DepartmentId where e.EmployeeID=".$exp['CrBy'], $con2); $un=mysql_fetch_assoc($u); ?>
		
		<tr onclick="showdet('<?=$exp['CrBy']?>')">
		  <td style="text-align:center;"><?=$i?></td>
		  <td style="text-align:center;"><?=$un['EmpCode']?></td>
		  <td><?=$un['Fname'].' '.$un['Sname'].' '.$un['Lname']?></td>
		  <td><?=$un['DepartmentName']?></td>
