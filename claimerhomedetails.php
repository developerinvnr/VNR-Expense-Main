...
while($res2=mysql_fetch_assoc($sql2)){ ?>		  
 ...
 <!--/***********************************/-->
 <!--/***********************************/-->
   <?php $sDl=mysql_query("select Driv_ExpiryDateTo,VehiclePolicy_EntryDate,VehiclePolicy_Type,Running_TotalKM from hrm_employee_personal p inner join hrm_employee_general g on p.EmployeeID=g.EmployeeID where g.EmployeeID=".$_SESSION['EmployeeID'],$con2); $rDl=mysql_fetch_assoc($sDl);   
   ...
   <?php if($_SESSION['EmployeeID']==1305){ ?>  <a class="btn btn-sm btn-primary" href="claim_oddm.php?vv=odo">&nbsp;&nbsp;<i class="fa fa-plus" aria-hidden="true"></i>&nbsp;odometer&nbsp;&nbsp;</a> <?php } ?>
   ...
   <?php if($_SESSION['EmployeeID']==169){ ?>
	&nbsp;&nbsp;
	<a href="duplicate.php" target="_blank" style="font-size:12px;">Duplicate</a>
	<?php } ?>
...
<?php /*********************************************************/ ?>	
	...
	<?php 
	if($_SESSION['FYearId']>=4)
	{ 
	 if($rDl['VehiclePolicy_Type']!='' OR ($rDl['VehiclePolicy_EntryDate']!='1970-01-01' AND $rDl['VehiclePolicy_EntryDate']!='0000-00-00' AND $rDl['VehiclePolicy_EntryDate']!='') OR $rDl['Running_TotalKM']>0)
	 { ?>
	<span style="font-size:14px;color:#004A00;cursor:pointer;" onclick="FunPocyOpen()">
     <b><u><i>My Vehicle Policy</i></u></b>&nbsp;<span class="blink_me"><img src="images/hand_point.png" width="18px" height="18px"/></span>
    </span>
	...
	<div style="font-size:14px;cursor:pointer;display:none; font-style:italic;" id="PolicyDiv">
	 ...
	 <div style="box-shadow:5px 5px 8px 5px #62C400;">
      <?php if($rDl['VehiclePolicy_Type']!=''){?>
	  &nbsp;<b style="vertical-align:middle;">*</b>&nbsp;Vehicle Policy Type : <font color="#FF0000"><?=$rDl['VehiclePolicy_Type']?></font><br/>
	  
	  <?php } if($rDl['VehiclePolicy_EntryDate']!='1970-01-01' AND $rDl['VehiclePolicy_EntryDate']!='0000-00-00' AND $rDl['VehiclePolicy_EntryDate']!=''){ ?>
      &nbsp;<b style="vertical-align:middle;">*</b>&nbsp;Vehicle Policy Entry Date : <font color="#FF0000"><?=date("d M Y",strtotime($rDl['VehiclePolicy_EntryDate']))?></font><br/> 
	  
	  <?php } if($rDl['Running_TotalKM']>0){ ?>
      &nbsp;<b style="vertical-align:middle;">*</b>&nbsp;Kms Claimed Upto Last Year : <font color="#FF0000"><?=intval($rDl['Running_TotalKM']).' Kms'?></font><br/>
	  
	  <?php } $sDl2=mysql_query("SELECT SUM(y1.`Totalkm`) as TotKM FROM `y".$_SESSION['FYearId']."_24_wheeler_entry` y1 inner join y".$_SESSION['FYearId']."_expenseclaims y2 on y1.ExpId=y2.ExpId where y2.CrBY=".$_SESSION['EmployeeID']." AND y2.ClaimStatus!='Deactivate' AND y2.ClaimAtStep>=5",$con); 
	        $rDl2=mysql_fetch_assoc($sDl2); if($rDl2['TotKM']>0){ ?>
      &nbsp;<b style="vertical-align:middle;">*</b>&nbsp;Kms Claimed For Current FY : <font color="#FF0000"><?=$rDl2['TotKM'].' Kms'?></font><br/>
	  <?php } ?>
...
<div class="row" style="padding-top:10px;">
 ...
 <table class="estable table shadow" style="width:100%;">
  ...
  <tbody>
  <?php $popupORnot="no"; 
  $m=mysql_query("SELECT `Month` FROM `y".$_SESSION['FYearId']."_monthexpensefinal` WHERE `EmployeeID`='".$_SESSION['EmployeeID']."' and YearId=".$_SESSION['FYearId']." and `Status`='Open' group by Month order by Month asc"); 
...
<table class="estable table shadow" style="width:100%;">
  ...
  <tbody>
  <?php $m=mysql_query("SELECT * FROM `y".$_SESSION['FYearId']."_monthexpensefinal` WHERE `EmployeeID`='".$_SESSION['EmployeeID']."' and YearId=".$_SESSION['FYearId']." and `Status`='Closed' and Total_Claim>0 group by Month order by Month asc");
   ...
   <tr>
	...
	<td style="width:60px;"><?php if($mlist['Approved_Amount']>0){echo date("d-m-y",strtotime($mlist['Approved_Date']));}?></td>	  
	<td style="width:50px;"><?php if($mlist['Fin_PayAmt']>0){ echo intval($mlist['Fin_AdvancePay']+$mlist['Fin_PayAmt']).'/-';} ?></td>
	<td style="width:60px;"><?php if($mlist['Fin_PayAmt']>0){ echo date("d-m-y",strtotime($mlist['Fin_PayDate']));}?></td>	
	<td style="width:100px;"><input style="border:hidden;width:100%;" value="<?php echo ucwords(strtolower($mlist['Fin_PayRemark']));?>"/></td>				
	
	<td style="text-align:center;">
	    
	    <button class="btn btn-sm btn-primary" style="font-size: 12px;float:center;" onclick="showMonthReport('<?=$_SESSION['EmployeeID']?>','<?=$mlist['Month']?>',<?=$_SESSION['FYearId']?>,2,2)"><i class="fa fa-table">&nbsp;Exp</i></button>
	</td>	
	<td style="text-align:center;">
	    <a href="printdetailsemp.php?e=<?=$_SESSION['EmployeeID']?>&m=<?=$mlist['Month']?>&y=<?=$_SESSION['FYearId']?>&n=2&nn=2"><button class="btn btn-sm btn-primary" style="font-size: 12px;float:center;"><i>Print</i></button></a>
	</td>
...
<script type="text/javascript">
 ...
 function showMonthReport(e,m,y,n,nn)
 {
  ...
  var modal = document.getElementById('myModal');
	...
	function expfilldeny(expid){ //alert("bb");
	...
	function deactivate(t,expid){ //alert("aa");
