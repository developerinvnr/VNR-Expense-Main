...
<?php
 ...
 if($ve>0 OR $_GET['sts']=='Submitted')
 {
   ...
   $q="SELECT e.*, CASE WHEN e.ClaimId!=0 THEN BillDate WHEN e.ClaimId = 0 THEN CrDate END AS finalDate FROM `y".$_SESSION['FYearId']."_expenseclaims` e, ".dbemp.".hrm_employee_general g where g.EmployeeID=e.CrBy and e.ClaimStatus!='Deactivate' ".$sts." and g.CostCenter in (".$state_data.") ".$sqr." group by e.ExpId order by finalDate, EmployeeID";
