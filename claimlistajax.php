	  ...
	  <tbody>
		...
		<?php 
			...
			if($_POST['cgid']==0 || $_POST['csts']=='Draft'){
				$q="SELECT e.CrDate as finalDate,e.* FROM `y".$_SESSION['FYearId']."_expenseclaims` e where e.ClaimMonth='".$_REQUEST['month']."' and e.ClaimYearId='".$_SESSION['FYearId']."' and  e.CrBy=".$_REQUEST['emp']."  and (e.ClaimStatus='Submitted' or e.ClaimStatus='Draft') and e.AttachTo=0 group by finalDate"; 
              ...
              // changes done feb 26 ef.BillDate
				...
				$q="SELECT e.*, c.ClaimName,e.BillDate as finalDate FROM `y".$_SESSION['FYearId']."_expenseclaims` e, claimtype c, `y".$_SESSION['FYearId']."_g".$_POST['cgid']."_expensefilldata` ef where e.ClaimMonth='".$_REQUEST['month']."' and  e.CrBy=".$_REQUEST['emp']." and (c.ClaimId=e.ClaimId) and c.cgId='".$_POST['cgid']."' and e.ClaimStatus!='Deactivate' and e.FilledBy>0  group by finalDate"; 
	...
	<table class="table shadow" style="width:100%;" align="center">
	  <thead class="thead-dark">
	    <tr>
			...
			<th scope="col" style="background-color:#008C8C;width:150px;"><font style="font-size:11px;">Status</th>
			<th scope="col" style="background-color:#008C8C;width:50px;"><font style="font-size:11px;"></th>
			<th scope="col" style="background-color:#008C8C;width:100px;"><font style="font-size:11px;">Remark</th>
	  ...
	  <tbody>
		...
		<?php 
			...
			if($_POST['cgid']==0 || $_POST['csts']=='Draft'){
				$q="SELECT e.* FROM `y".$_SESSION['FYearId']."_expenseclaims` e where e.ClaimYearId='".$_SESSION['FYearId']."' and e.CrBy=".$_REQUEST['emp']." and e.CrDate='".$_POST['date']."' and e.AttachTo=0 and (e.ClaimStatus='Submitted' or e.ClaimStatus='Draft') group by e.ExpId"; 
				...
				$q="SELECT e.*, c.ClaimName FROM `y".$_SESSION['FYearId']."_expenseclaims` e, claimtype c, `y".$_SESSION['FYearId']."_g".$_POST['cgid']."_expensefilldata` ef where e.ExpId=ef.ExpId and e.ClaimYearId='".$_SESSION['FYearId']."' and  e.CrBy=".$_REQUEST['emp']." and (c.ClaimId=e.ClaimId) and c.cgId='".$_POST['cgid']."' and e.BillDate='".$_POST['date']."' and e.AttachTo=0 and e.FilledBy>0 and e.ClaimStatus!='Deactivate' and e.ClaimStatus!='Draft' group by e.ExpId"; 
			}
...
<td style="background-color:#CCCCCC;"></td>
 <td colspan="10">
	...
	<?php $qq="SELECT Fname,Sname,Lname FROM hrm_employee where EmployeeID=".$_REQUEST['emp']; 
	$seleqq=mysql_query($qq,$con2); $expn=mysql_fetch_assoc($seleqq); ?>
	  ...
	  <tbody>
		...
		*/
	  	...
	  	$q="SELECT e.*, c.ClaimName

	  	CASE WHEN e.ClaimId != 0  THEN BillDate 
		WHEN e.ClaimId = 0  THEN CrDate 
		END AS finalDate 

	  	 FROM `y".$_SESSION['FYearId']."_expenseclaims`e, claimtype c where e.ClaimYearId='".$_SESSION['FYearId']."' and  e.CrBy=".$_REQUEST['emp']." and (c.ClaimId=e.ClaimId or e.ClaimId=0) and ".$stepcond." and e.AttachTo=0  group by e.ExpId order by finalDate";
	...
	<table class="table shadow">
	  ...
	  <tbody>
		...
		$crcond="1=1";
	  	...
	  	$q="SELECT e.*, c.ClaimName FROM `y".$_SESSION['FYearId']."_expenseclaims`e, claimtype c where c.ClaimId=e.ClaimId and e.ClaimMonth='".$_REQUEST['month']."' and e.ClaimYearId='".$_SESSION['FYearId']."' and  e.CrBy=".$_REQUEST['emp']." and e.FilledOkay=1 and e.ClaimStatus!='Deactivate' and e.ClaimStatus!='Draft' and ".$stepcond." order by BillDate ASC";
	    ...
	    <tr onclick="showdet('<?=$exp['EmployeeID']?>')">
	      ...
	      <td><?php echo $un['Fname']."&nbsp".$un['Sname']."&nbsp".$un['Lname']; //echo getUser($exp['CrBy']); ?></td>
	      <td><?=$exp['ClaimName']?></td>   
...
<?php
if($_REQUEST['csts']=='Verified'){ $stepcond="e.ClaimAtStep>=3"; }
elseif ($_REQUEST['csts']=='Approved'){ $stepcond="e.ClaimAtStep>=5"; }
$crcond="1=1";
$q="SELECT e.*, c.ClaimName FROM `y".$_SESSION['FYearId']."_expenseclaims`e, claimtype c where c.ClaimId=e.ClaimId and e.ClaimMonth='".$_REQUEST['month']."' and e.ClaimYearId='".$_SESSION['FYearId']."' and  e.CrBy=".$_REQUEST['emp']." and e.FilledOkay=1 and e.ClaimStatus!='Deactivate' and e.ClaimStatus!='Draft' and ".$stepcond." order by BillDate ASC"; $seleq=mysql_query($q);
...
<?php $famt=intval($exp['FilledTAmt']); $vamt=intval($exp['VerifyTAmt']);
  ...
  <td <?=$vcstyle?> class="text-right"><?php if($exp['ClaimAtStep']>=4){echo $exp['VerifyTAmt']; }?></td>
  
  <td class="text-left"><?=$exp['VerifyTRemark']?></td>
  
  <td><input style="width:100%;" class="form-control text-right" type="text" id="<?=$exp['ExpId']?>apprtamt" value="<?php if($exp['ClaimAtStep']>=4){echo $exp['ApprTAmt']; }else{ echo 0; }?>" onkeypress="return isNumber(event)" onkeyup="checkrange(this,'<?=$exp['FilledTAmt']?>')"></td>
